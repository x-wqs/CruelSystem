...
type Raft struct {
	...
	// https://www.cnblogs.com/mignet/p/6824_Lab_2_Raft_2C.html
...
func (rf *Raft) getRaftState() []byte {
	...
	encoder.Encode(rf.votee)
	encoder.Encode(rf.offset)
	encoder.Encode(rf.lastLogIndex)
...
func (rf *Raft) readPersist(data []byte) {
	...
	var term, votee, offset, lastLogIndex int
	if decoder.Decode(&term) != nil ||
		decoder.Decode(&votee) != nil ||
		decoder.Decode(&offset) != nil ||
		decoder.Decode(&lastLogIndex) != nil {
		panic("Failed to decoe Raft state\n")
	} else {
		logs := make([]LogEntry, lastLogIndex - offset)
		if decoder.Decode(&logs) != nil {
			panic("Failed to read persis logs\n")
		} else {
			rf.term = term
			rf.votee = votee
			rf.offset = offset
...
func (rf *Raft) CondInstallSnapshot(lastIncludedTerm int, lastIncludedIndex int, snapshot []byte) bool {
	...
	if lastIncludedIndex <= rf.lastApply {
		// TODO-log-error
		return false
	}

	// TODO-log-info
	rf.persistBeforeSnapshot(lastIncludedIndex, lastIncludedTerm, snapshot)
	return true
}

func (rf *Raft) persistBeforeSnapshot(index, term int, snapshot []byte) {
	// todo-log-info
	if index > rf.commitIndex {
		rf.commitIndex = index
	}
	if index > rf.lastApply {
		rf.lastApply = index
	}
	n = rf.lastLogIndex - index + 1
	if n < 1 {
		n = 1
	...
	// todo-log-info
	// todo-log: no snapshot when lastApply not updated ?
}

// https://blog.csdn.net/qq_40443651/article/details/116091524
...
func (rf *Raft) InstallSnapshot(request *SnapshotRequest, response *SnapshotResponse) {
	rf.mu.Lock()
	defer func() {
		rf.mu.Unlock()
	...
	response.Term = rf.term
	if request.LastSnapshotIndex <= rf.lastApply {
		return
	}
	...
	go func() {
		rf.applyCh <- ApplyMsg{
			...
			SnapshotTerm: 	request.LastSnapshotTerm,
			SnapshotIndex: 	request.LastSnapshotIndex,
		}
	}()
}

func (rf *Raft) setNewerTerm(term int) {
	rf.state = FOLLOWER
	rf.voted = 0
