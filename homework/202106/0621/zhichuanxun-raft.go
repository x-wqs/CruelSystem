...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	if rf.lastLogIndex + 1 >= len(rf.logs) {
		rf.logs = append(rf.logs, RFLog{rf.term, rf.lastLogIndex + 1, command})
	} else {
		rf.logs[rf.lastLogIndex+1] = RFLog{rf.term, rf.lastLogIndex + 1, command}
	}
	rf.lastLogIndex++
	rf.lastLogTerm = rf.term
	rf.committedCounter[rf.lastLogIndex]++
	// self commit confirmed
	// rf.applyLog(rf.lastLogIndex)
	// rf.commitBefore()
	// DPrintf("%d", rf.lastLogIndex)

	return rf.lastLogIndex, rf.term, isLeader
}
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term == rf.term {
		if rf.voted || args.LastLogTerm < rf.lastLogTerm || (args.LastLogTerm == rf.lastLogTerm && args.LastLogIndex < rf.lastLogIndex) {
...
func (rf *Raft) syncFollowerLogs(serverId int) {
	for {
		...
		// DPrintf("current nextIdx %d, last log %d", rf.nextIndexes[serverId], rf.lastLogIndex)
		if rf.nextIndexes[serverId] <= rf.lastLogIndex {
			DPrintf("%d to %d", rf.me, serverId)
			rf.CallAppendEntries(serverId, false)
			// rpc has less interval
			time.Sleep(time.Duration(HEARTBEAT_INTERVAL/2) * time.Millisecond)
		} else {
			// no entry need to append
			// send heartbeat
			// in case of leader's suicide
			rf.CallAppendEntries(serverId, true)
			time.Sleep(time.Duration(HEARTBEAT_INTERVAL) * time.Millisecond)
		}
	}
}
...
func (rf *Raft) CallAppendEntries(serverId int, isHeartbeat bool) {
	...
	DPrintf("+++++++++++++++++++")
	DPrintf("%d - %d - %d", rf.me, rf.lastLogIndex, rf.committedIndex)
	for i := 0; i < len(rf.logs); i++ {
		DPrintf("cmd:%d idx:%d term:%d", rf.logs[i].Command, rf.logs[i].Index, rf.logs[i].Term)
	}
	DPrintf("+++++++++++++++++++")
	...
	DPrintf("%d call %d apply %d %t", rf.me, serverId, tryIndex, reply.Success)
	...
	if !reply.Success {
		...
		// DPrintf("%t fail from %d to %d: previdx: %d pervterm %d", isHeartbeat, rf.me, serverId, args.PrevLogIndex, args.PrevLogTerm)
		// // if !isHeartbeat {
		// 	//TODO: make heartbeat works as well
		// // there is a bug during tests since resource not be clean up totally
		// for i:=0; i < len(rf.peers); i++ {
		// 	DPrintf("|||%d", rf.nextIndexes[i])
		// }
		// DPrintf("---%d %d %d", rf.lastLogIndex, rf.leaderCommit, len(rf.logs))
		rf.nextIndexes[serverId]--
		DPrintf("%d to %d", serverId, rf.nextIndexes[serverId])
	} else {
		if !isHeartbeat {
			// if not a heartbeat, means an entry applied
			// update next idx
			rf.nextIndexes[serverId] = tryIndex + len(args.Entries)
			// update commit counter
			appendedIndex := tryIndex + len(args.Entries) - 1
			DPrintf("[%d, %d, %d]", tryIndex, appendedIndex, rf.committedIndex)
			if rf.committedIndex < appendedIndex {
				// TODO:merge these three loop?
				for i := tryIndex; i <= appendedIndex; i++ {
					rf.committedCounter[i]++
				}
				newCommitted := rf.committedIndex
				for i := rf.committedIndex + 1; i <= appendedIndex; i++ {
					if rf.committedCounter[i] > len(rf.peers)/2 {
						newCommitted = i
					}
				}
				// apply entries
				for i := rf.committedIndex + 1; i <= newCommitted; i++ {
					rf.applyLog(i)
				}
				rf.committedIndex = newCommitted
				DPrintf("change to %d", rf.committedIndex)
			}
		}
	}
}
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	// ########## log replication ##########
	// DPrintf("prevIdx %d, prevTerm %d", args.PrevLogIndex, args.PrevLogTerm)
	reply.Term = rf.term
	...
	// DPrintf("%d index%d  %d, term: %d %d", len(args.Entries), args.PrevLogIndex, rf.lastLogIndex, args.PrevLogTerm, rf.lastLogTerm)

	// if len(args.Entries) > 0 {
	// 	//DPrintf("%d received", rf.lastLogIndex)
	// }
	...
	if args.PrevLogTerm == rf.logs[args.PrevLogIndex].Term {
		...
		if len(args.Entries) > 0 {
			...
			//DPrintf("!!!!!%d apply %d", rf.me, len(args.Entries))
			for i := 0; i < len(args.Entries); i++ {
				newLog := args.Entries[i]
				if len(rf.logs) <= newLog.Index {
		...
		if rf.committedIndex < args.LeaderCommit {
			for i := rf.committedIndex + 1; i <= min(args.LeaderCommit, len(rf.logs)-1); i++ {
				rf.applyLog(i)
			}
			rf.committedIndex = args.LeaderCommit
			//DPrintf("%d commit %d", rf.me, rf.committedIndex)
		}
		return
	}
	reply.Success = false
...
func (rf *Raft) applyLog(logIdx int) {
	...
	DPrintf("%d apply %d", rf.me, logIdx)
	rf.applyCh <- msg
	// DPrintf("%d apply %d", rf.me, logIdx)
}
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	// !!! ignore out-of-date rpc between tests
	time.Sleep(time.Duration(TIMEOUT_BASE+rand.Int31n(TIMEOUT_RANGE)) *
		time.Millisecond)
	rf := &Raft{}
	rf.peers = peers
