...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	rf.logs = append(rf.logs, RFLog{rf.lastLogTerm, rf.lastLogIndex, command})
...
func (rf *Raft) syncFollowerLogs(serverId int) {
	for {
		...
		if rf.nextIndexes[serverId] <= rf.lastLogIndex {
			...
			time.Sleep(time.Duration(HEARTBEAT_INTERVAL/4) * time.Millisecond)
		} else {
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.PrevLogIndex > rf.lastLogIndex {
		...
		DPrintf("================ %d %d", args.PrevLogIndex, rf.lastLogIndex)
		reply.Success = false
		return
	}
	...
	if args.PrevLogTerm == rf.logs[args.PrevLogIndex].Term {
		...
		if rf.committedIndex < args.LeaderCommit {
			for i := rf.committedIndex + 1; i < min(args.LeaderCommit+1, len(rf.logs)); i++ {
				rf.applyLog(i)
			}
		}
